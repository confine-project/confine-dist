	...
	for line in io.lines(sys_conf.ssh_node_auth_file) do
		...
		if user_id then
			...
			if tonumber(id) and user_key and user_key:find("^%s"%ssl.SSH_HEADER) then
				...
				del_ssh_keys(sys_conf.ssh_node_auth_file)
				group.user_roles = {}
				break
			end
		end
...
function cb2_set_node_lgroup( rules, sys_conf, otree, ntree, path, begin, changed )
	if not rules then return "cb2_set_node_lgroup" end
...
function cb2_set_node_lgroup_role( rules, sys_conf, otree, ntree, path, begin, changed )
	if not rules then return "cb2_set_node_lgroup_role" end
	...
	if begin and old and not new then
		...
		del_ssh_keys(sys_conf.ssh_node_auth_file, user_id)
	...
	elseif not begin and changed and new then
		...
		if (new.is_technician or new.is_admin) and new.local_user and new.local_user.is_active then
			...
			if ctree.iterate(function() end, {[1]={"/*", function() end}}, sys_conf, old_tokens, new_tokens, "/") then --just scan for changes
				...
				add_ssh_keys(sys_conf.ssh_node_auth_file, user_id, new_tokens)
			end
